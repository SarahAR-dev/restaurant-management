...
export default function DrinkDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  ...
  const loadDrinkDetails = async () => {
    try {
      ...
      const foundDrink = data.find((d: Drink) => d.id === id)
  ...
  const handleAddReview = async (review: string) => {
    ...
    try {
      const updatedReviews = [...(drink.reviews || []), review]
  ...
  const handleDeleteReview = async (index: number) => {
    ...
    try {
      const updatedReviews = drink.reviews?.filter((_, i) => i !== index) || []
  ...
  if (!drink) {
    return (
      <div className="p-8">
        <p className="text-center">Boisson introuvable</p>
        <Button onClick={() => router.push("/drinks")} className="mt-4">
          Retour aux boissons
        </Button>
      </div>
    )
  ...
  // Filtrer les commandes contenant cette boisson
  const ordersWithThisDrink = orders
    .filter(order => order.items.some(item => item.name === drink.name))
  ...
  const todayOrders = ordersWithThisDrink.filter(order => 
    new Date(order.createdAt).toISOString().split('T')[0] === today
  )

  const weekOrders = ordersWithThisDrink.filter(order => 
    new Date(order.createdAt) >= weekAgo
  )

  const totalQuantity = ordersWithThisDrink.reduce((sum, order) => {
    const item = order.items.find(i => i.name === drink.name)
  ...
  const totalRevenue = totalQuantity * drink.price
  const todayRevenue = todayQuantity * drink.price
  const weekRevenue = weekQuantity * drink.price
    ...
    <div className="space-y-6 p-6">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              ...
              <div className="space-y-2 max-h-40 overflow-y-auto">
                {(!drink.reviews || drink.reviews.length === 0) ? (
                  ...
                  drink.reviews.map((review: string, index: number) => (
                    <div 
                      key={index} 
      ...
      <div className="bg-card border rounded-lg p-6">
        <h2 className="text-xl font-bold mb-4">
          🛒 Dernières commandes ({ordersWithThisDrink.length > 5 ? '5' : ordersWithThisDrink.length} plus récentes)
        </h2>
        {ordersWithThisDrink.length === 0 ? (
          <p className="text-muted-foreground text-center py-8">
            Aucune commande contenant cet article
          </p>
        ) : (
          <div className="space-y-3">
            {ordersWithThisDrink.map((order) => {
              const item = order.items.find(i => i.name === drink.name)
              return (
                <div
                  key={order.id}
